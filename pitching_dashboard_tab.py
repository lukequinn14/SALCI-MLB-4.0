...
def get_team_logo_url(team: str, dark_bg: bool = False) -> str:
    ...
    dark_bg : When True AND the team is in _DARK_BACKGROUND_TEAMS, returns
              ESPN's /500-dark/ path (higher-contrast variant for dark chart
              backgrounds, e.g. white NY for Yankees, yellow SD for Padres).
              When False (white-pill HTML contexts), always returns the standard
              /500/scoreboard/ logo so dark logos are visible on white.
    ...
    # Use /500-dark/ ONLY on dark chart backgrounds (dark_bg=True) AND for teams
    # whose primary logo is invisible on dark backgrounds.
    # White-pill HTML contexts always pass dark_bg=False → standard scoreboard logo.
...
def resolve_logo_url(team: str, cached_url: str | None, dark_bg: bool = False) -> str:
    ...
    if cached_url and ESPN_HOST in cached_url:
        ...
        abbrev = _resolve_abbrev(team)
        needs_dark = dark_bg and abbrev in _DARK_BACKGROUND_TEAMS

        if needs_dark:
            # Chart context: we want /500-dark/ — if cached URL isn't dark, re-resolve
            if "/500-dark/" in url_lower:
                return cached_url
            else:
                return get_team_logo_url(team, dark_bg=True)
        else:
            # White-pill / HTML context: we want /500/scoreboard/
            # If cached URL is a dark variant, re-resolve to get the standard one
            if "/500-dark/" in url_lower:
